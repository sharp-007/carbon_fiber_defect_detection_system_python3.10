...
carbon_fiber_defect_detection_system/
  ...
  ├─ log/                   # 日志目录（运行Streamlit应用时生成）
  ├─ test_image_video/      # 运行Streamlit应用的测试图片和视频目录
  ├─ test_demo/             # 演示结果目录
  │  ├─ streamlit_result_demo/  # Streamlit应用演示结果
  │  │  ├─ 图片模式/        # 图片检测演示结果
  │  │  │  ├─ *.csv        # 检测结果CSV文件
  │  │  │  └─ *.pdf        # 检测结果PDF报告
  │  │  ├─ 视频模式/        # 视频检测演示结果
  │  │  │  ├─ *.mp4        # 带检测框的视频文件
  │  │  │  ├─ *.csv        # 检测结果CSV文件
  │  │  │  ├─ video_keyframes/  # 关键帧图片
  │  │  │  ├─ *.png        # 缺陷时间分布图
  │  │  │  └─ *.pdf        # 检测结果PDF报告
  │  │  └─ 摄像头模式/      # 摄像头实时检测演示结果
  │  │     ├─ *.csv        # 检测结果CSV文件
  │  │     ├─ camera_keyframes/  # 关键帧图片
  │  │     ├─ *.png        # 缺陷时间分布图
  │  │     └─ *.pdf        # 检测结果PDF报告
  │  └─ streamlit_video_demo/   # Streamlit应用演示视频
  │     └─ *.mp4           # 产品演示视频
  ├─ test_results/          # 测试集评估结果（运行test.py后生成，根目录下的单独文件夹）
  │  ├─ test_summary.json   # 测试结果摘要
  │  ├─ *.png               # 测试图表（混淆矩阵、PR曲线等）
  │  └─ test_visualizations/  # 测试集可视化对比图片
  └─ runs/                  # 训练输出目录（训练后生成）
     └─ run1/               # 第1次训练结果（每次训练创建新的run文件夹）
        ├─ train/           # 训练结果目录
        │   ├─ weights/     # 模型权重文件（best.pt, last.pt）
        │   ├─ args.yaml    # 训练参数配置
        ...
        └─ val/             # 验证结果目录
            ...
            ├─ *.png        # 验证图表（混淆矩阵、PR曲线等）
            └─ validation_visualizations/  # 验证集可视化对比图片
```
...
- `streamlit>=1.18.0`：Web 应用框架（需要 1.18.0+ 以支持 `st.cache_resource`）
- `ultralytics>=8.0.0`：YOLO 模型库（核心依赖，详见第 7 节）
- `opencv-python-headless>=4.5`：图像处理（OpenCV 无 GUI 版本）
- `numpy>=1.23`：数值计算库
- `Pillow>=9.0`：图像处理库（PIL）
- `pandas>=1.5`：数据处理和分析
- `matplotlib>=3.5.0`：绘图库（用于生成统计图表）
- `altair>=4.2.0`：交互式数据可视化库
- `pyyaml>=6.0`：YAML 配置文件解析（用于读取 data.yaml）

> **注意**：`ultralytics` 会自动安装其依赖项（如 `torch`、`torchvision` 等）。如果需要 GPU 支持，请确保安装了 CUDA 版本的 PyTorch。

### 8.1 Streamlit Cloud 部署
